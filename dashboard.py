...
try:
    ...
    st.error("❌ Could not load Streamlit secrets")
    st.write(e)
    st.stop()
...
sport_options = sorted(df["sport"].dropna().unique().tolist()) if "sport" in df.columns else []
...
book_options = sorted(df["sportsbook"].dropna().unique().tolist()) if "sportsbook" in df.columns else []
